...
contract MyToken is ERC20, ERC20Burnable, Ownable {
	...
	function withdraw(uint256 tttAmount) public{
		...
		require(address(this).balance >= ethToTransfer, "Not enough ETH in the contract");
		require(balanceOf(msg.sender) >= tttAmount, "Not enough TTT tokens to withdraw");
    ...
    function transferByUsername(string memory username, uint256 amount) public  {
        require(balanceOf(msg.sender) >= amount, "Sender does not have enough TTT to send");
	...
	function getTiktokBalance(string memory tiktokusername) external view returns (uint) {
	...
	function _beforeTokenTransfer(
        ...
        require(to != address(this), "Cannot transfer tokens to the contract itself");

		require(!isContract(to), "Cannot transfer tokens to another contract address");
	}
}
